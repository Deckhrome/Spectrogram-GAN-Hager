...
for filename in os.listdir(source_dir):
    if filename.endswith('.png'):
        ...
        if 'current' in filename.lower():
            shutil.move(os.path.join(source_dir, filename), os.path.join(current_dir, filename))
        # Trier les fichiers contenant "voltage" dans leur nom
        elif 'voltage' in filename.lower():
            shutil.move(os.path.join(source_dir, filename), os.path.join(voltage_dir, filename))
